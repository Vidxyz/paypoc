  ...
  def openApiJson: Action[AnyContent] = Action {
    try {
      ...
      val swaggerJson = (
        ...
        """{"openapi":"3.0.0","info":{"title":"User Service API","version":"1.0.0","description":"Swagger spec not found. Ensure 'sbt swagger' runs during build."},"paths":{}}"""
      }
      ...
      case e: Exception =>
        ...
        Ok(s"""{"openapi":"3.0.0","info":{"title":"User Service API","version":"1.0.0","description":"Error loading swagger.json: ${e.getMessage}"},"paths":{}}""").as("application/json")
    }
  }
}
